...
def reflect_table_columns(engine, table_name, conn=None):
    """Reflect existing table columns as a mapping column_name -> sql type string.

    If `conn` (a SQLAlchemy Connection) is provided, use it for reflection so
    uncommitted DDL inside the same transaction is visible.
    """
    md = MetaData()
    try:
        target = conn if conn is not None else engine
        table = Table(table_name, md, autoload_with=target)
    except Exception:
        return {}
    cols = {}
